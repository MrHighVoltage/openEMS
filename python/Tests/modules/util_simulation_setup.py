        ...
        if metal == None: # this layer does not exist in XML stackup
            ...
            if poly.layernum in simulation_ports.portlayers:
                ...
                if port != None:
                    ...
                    port_edges2grid = None
                    if port_mode == "compact_gap":
                        # The driver extends both physical terminals to a short
                        # central gap.  Place the lumped source only across that
                        # gap, rather than integrating voltage over the original
                        # long GDS port marker.
                        if port_direction != "y":
                            raise ValueError(
                                "compact_gap port mode currently supports y-directed ports only"
                            )
                        span = float(os.environ.get("OPENEMS_PORT_GAP_LENGTH", "1.0"))
                        if span <= 0:
                            raise ValueError("OPENEMS_PORT_GAP_LENGTH must be positive")
                        marker_length = abs(ymax - ymin)
                        if span >= marker_length:
                            raise ValueError(
                                "OPENEMS_PORT_GAP_LENGTH must be shorter than the GDS port-marker length"
                            )
                        y_center = 0.5 * (ymin + ymax)
                        if port.reversed_direction:
                            ymin = y_center + 0.5 * span
                            ymax = y_center - 0.5 * span
                        else:
                            ymin = y_center - 0.5 * span
                            ymax = y_center + 0.5 * span
                        port_edges2grid = "xy"
                    elif port_mode in ("centerline", "compact"):
                        # Keep a connected lumped port for a large terminal gap,
                        # but restrict its transverse footprint to a narrow
                        # source strip.  Cropping the excitation direction
                        # would disconnect the port from one of the terminals.
                        if port_direction != "y":
                            raise ValueError(
                                "centerline/compact port mode currently supports y-directed ports only"
                            )
                        span = float(os.environ.get("OPENEMS_PORT_TRANSVERSE_SPAN", "1.0"))
                        if span <= 0:
                            raise ValueError("OPENEMS_PORT_TRANSVERSE_SPAN must be positive")
                        marker_width = abs(xmax - xmin)
                        if span > marker_width:
                            raise ValueError(
                                "OPENEMS_PORT_TRANSVERSE_SPAN exceeds the GDS port width"
                            )
                        x_center = 0.5 * (xmin + xmax)
                        xmin = x_center - 0.5 * span
                        xmax = x_center + 0.5 * span
                        port_edges2grid = "xy"
                    elif port_mode == "localized":
                        # Diagnostic mode: a GDS port marker may cover a complete
                        # feed line. Limit the lumped source to one short segment
                        # while preserving its original transverse dimensions.
                        # This is intentionally opt-in because it may disconnect
                        # the source from the far terminal.
                        span = float(os.environ.get("OPENEMS_PORT_LOCAL_SPAN", "1.0"))
                        if span <= 0:
                            raise ValueError("OPENEMS_PORT_LOCAL_SPAN must be positive")
                        if port.reversed_direction:
                            y_start = ymax
                            y_stop = max(ymin, ymax - span)
                        else:
                            y_start = ymin
                            y_stop = min(ymax, ymin + span)
                        ymin = y_start
                        ymax = y_stop
                        port_edges2grid = "xy"

                    port_options = {"priority": 150}
                    if port_edges2grid is not None:
                        port_options["edges2grid"] = port_edges2grid
                    CSX_port = FDTD.AddLumpedPort(portnum, port_Z0, [xmin, ymin, zmin], [xmax, ymax, zmax], port_direction, voltage, **port_options)
...
def runSimulation (excite_portnumbers, FDTD, sim_path, model_basename, preview_only, postprocess_only, force_simulation=False, engine=None, run_options=None):
    ...
    if not postprocess_only:
        ...
        # preview model
        if os.environ.get("OPENEMS_SHOW_CSXCAD", "0") == "1" and 1 in excite_portnumbers:
    ...
    if not (preview_only or postprocess_only):  # start simulation
        ...
        if (existing_data_hash != XML_hash) or force_simulation:
            ...
            try:
                failure_marker = os.path.join(excitation_path, 'openEMS_numerical_failure.txt')
                ...
                FDTD.Run(excitation_path, **options)

                if os.path.exists(failure_marker):
                    with open(failure_marker, 'r') as marker_file:
                        reason = marker_file.read().strip()
                    if not reason:
                        reason = 'openEMS reported an unspecified numerical failure'
                ...
                print('[ERROR] FDTD simulation cancelled: ', e)
                sys.exit(1)
        else:
