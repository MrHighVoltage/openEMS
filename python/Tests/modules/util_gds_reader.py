...
def read_gds(filename, layerlist, purposelist, metals_list, preprocess=False, merge_polygon_size=0 ):
  ...
  if os.path.isfile(filename):
    ...
    # Select the top-level cell that best matches the requested stackup and port
    # layers. GDS exports may include a separate context cell before the model.
    toplevel_cell_list = input_library.top_level()
    requested_layers = set(layerlist)
    cell = max(
      toplevel_cell_list,
      key=lambda candidate: len(requested_layers.intersection(candidate.get_layers()))
    )
    print('Using top-level GDSII cell:', cell.name)
