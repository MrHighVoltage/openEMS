...
bool HDF5_File_Writer::OpenFile()
{
	...
	if (m_h5file < 0)
	{
		cerr << "HDF5_File_Writer::OpenFile: Error, opening file \"" << m_filename << "\" failed" << endl;
		return false;
	}
	return true;
}
...
hid_t HDF5_File_Writer::AcquireFile()
{
	if (m_h5file >= 0)
		return m_h5file;
	return H5Fopen(m_filename.c_str(), H5F_ACC_RDWR, H5P_DEFAULT);
}

void HDF5_File_Writer::ReleaseFile(hid_t file)
{
	if (file != m_h5file && file >= 0)
		H5Fclose(file);
}
...
bool HDF5_File_Writer::WriteRectMesh(unsigned int const* numLines, double const* const* discLines, int MeshType, double scaling, std::string s_mesh_grp)
{
	...
	WriteAttribute(mesh_grp, "mesh_type", &MeshType, 0, H5T_NATIVE_INT);
	WriteAttribute(mesh_grp, "mesh_scaling", &scaling, 0, H5T_NATIVE_DOUBLE);
...
bool HDF5_File_Writer::WriteData(hid_t group, std::string dataSetName, hid_t mem_type, void const* field_buf, size_t dim, size_t* datasize, std::string d_order)
{
	...
	if (!d_order.empty())
	{
		...
		WriteAttribute(dataset, "d_order", &s_ptr, 0, tid);
		H5Tclose(tid);
	}
...
bool HDF5_File_Writer::WriteAttribute(hid_t loc, std::string attr_name, void const* value, hsize_t size, hid_t mem_type)
{
	hid_t dataspace_id;
	if (size>0)
	...
	if (attribute_id<0)
	{
		cerr << "HDF5_File_Writer::WriteAttribute: Error, failed to create the attribute: \"" << attr_name << "\"" << endl;
	...
	{
		cerr << "HDF5_File_Writer::WriteAttribute: Error, failed to write the attribute: \"" << attr_name << "\"" << endl;
...
bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, float const* value, hsize_t size)
{
	return WriteAttribute(locName,attr_name, value, size, H5T_NATIVE_FLOAT);
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, double const* value, hsize_t size)
{
	return WriteAttribute(locName,attr_name, value, size, H5T_NATIVE_DOUBLE);
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, vector<float> values)
{
	float* val = new float[values.size()];
	for (size_t n=0;n<values.size();++n)
		val[n]=values.at(n);
	bool ok = HDF5_File_Writer::WriteAttribute(locName, attr_name,val,values.size(),H5T_NATIVE_FLOAT);
	delete[] val; val=NULL;
	return ok;
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, vector<double> values)
{
	double* val = new double[values.size()];
	for (size_t n=0;n<values.size();++n)
		val[n]=values.at(n);
	bool ok =  HDF5_File_Writer::WriteAttribute(locName, attr_name, val, values.size(), H5T_NATIVE_DOUBLE);
	delete[] val; val=NULL;
	return ok;
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, vector<int> values)
{
	int* val = new int[values.size()];
	for (size_t n=0;n<values.size();++n)
		val[n]=values.at(n);
	bool ok =  HDF5_File_Writer::WriteAttribute(locName, attr_name, val, values.size(), H5T_NATIVE_INT);
	delete[] val; val=NULL;
	return ok;
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, vector<unsigned int> values)
{
	unsigned int* val = new unsigned int[values.size()];
	for (size_t n=0;n<values.size();++n)
		val[n]=values.at(n);
	bool ok =  HDF5_File_Writer::WriteAttribute(locName, attr_name, val, values.size(), H5T_NATIVE_UINT);
	delete[] val; val=NULL;
	return ok;
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, float value)
{
	return HDF5_File_Writer::WriteAttribute(locName, attr_name, &value, 0, H5T_NATIVE_FLOAT);
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, double value)
{
	return HDF5_File_Writer::WriteAttribute(locName, attr_name, &value, 0, H5T_NATIVE_DOUBLE);
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, int value)
{
	return HDF5_File_Writer::WriteAttribute(locName, attr_name, &value, 0, H5T_NATIVE_INT);
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, unsigned int value)
{
	return HDF5_File_Writer::WriteAttribute(locName, attr_name, &value, 0, H5T_NATIVE_UINT);
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, size_t value)
{
	return HDF5_File_Writer::WriteAttribute(locName, attr_name, &value, 0, H5T_NATIVE_HSIZE);
}
...
bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, std::string value)
{
	...
	bool ok = HDF5_File_Writer::WriteAttribute(locName, attr_name, &s_ptr, 0, tid);
	H5Tclose(tid);
	return ok;
}

bool HDF5_File_Writer::WriteAttribute(std::string locName, std::string attr_name, bool value)
{
	return HDF5_File_Writer::WriteAttribute(locName, attr_name, &value, 0, H5T_NATIVE_HBOOL);
}
